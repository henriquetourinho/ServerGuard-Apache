...
# Função para verificar processos críticos (exemplo: Apache e Nginx)
check_process() {
...
PROCESS_APACHE=$(check_process "apache2")
if (( PROCESS_APACHE == 0 )); then
  send_alert "Alerta: O processo Apache não está em execução!"
fi

# Verificar processo Nginx
PROCESS_NGINX=$(check_process "nginx")
if (( PROCESS_NGINX == 0 )); then
  send_alert "Alerta: O processo Nginx não está em execução!"
fi
